...
| `interaction` | Direct-input panel on number sliders, select-backed sliders (min/max current), battery boost and the plan target; keyboard writes (none for a key that moved nothing); outside click (inside and outside the card) / tab switch closing the panel, a deferred hass update surviving a panel switch; `hide_settings`, `slider_steps`; plan preview request |
