...
| `interaction` | Direct-input panel on number sliders, select-backed sliders (min/max current), battery boost and the plan target; keyboard writes (none for a key that moved nothing); outside click (inside and outside the card) / tab switch closing the panel, a deferred hass update surviving a panel switch; `hide_settings`, `slider_steps` (matched against the feature key, not the tail of the entity id, and reported in the console when a select-backed slider cannot honour it); plan preview request; more-info on the loadpoint header values (status, remaining time, vehicle, SoC, power, currents, session figures) in loadpoint and compact mode, with the slider values kept out and the attribute surviving a live update; the mode buttons mark the pressed one before HA answers and give the mark back when the service call is refused; a focused date input or select and a slider drag hold a render back whoever asks for it (the element survives a direct `_render()`), blur, change and pointerup run the deferred render, and a re-mount clears the guard |
