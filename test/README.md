...
| Group | Checks |
|---|---|
| `unit` | Pure functions from `src/utils` and `src/core` in node, no browser: vehicle discovery and the device link (role assignment, device search, title from ha-evcc's vehicle device), number/step formatting, the dual-format evcc timestamp parser, duration and countdown strings, the SoC gradients, HTML escaping, the `stats_period` normalisation, entity discovery, and that `package.json` still carries the version from `src/core/constants.js` |
...
| `vehicle` | Vehicle mode: a block per vehicle evcc knows, plugged in or not; charge level, range and odometer read from the loadpoint while the vehicle is connected and from its own sensors otherwise, each value opening the entity it came from; a vehicle evcc cannot reach shows a hint instead of 0 %; repeating plans and session totals per vehicle, the plan toggle calling `switch.turn_on`; the loadpoint's charge plan read only at the vehicle plugged in there; the `vehicles` filter as a list and as a single name, an unknown name listing the vehicles that exist; the hint when the extended vehicle data is off; vehicles of a second installation staying out; the device of the vehicle's own integration (`fixtures/vehicle_device.json`, modelled on a real car with 72 entities): found on its own and not confused with the companion app's device of the same name, evcc's loadpoint values leading while connected and the value that changed last winning otherwise, range not mistaken for the distance to service, chips for lock, open lids, warnings and location, the folded detail list, `vehicle_devices` as override, `none` and `false`, and a change of an entity without the evcc prefix reaching the card through the render key; the picture in its four states (parked, driving, connected, charging) with badge and text alternative, evcc leading at its loadpoints and the integration telling the rest, charging away from home, an unknown charge level leaving the battery empty, `vehicle_graphic: hide`; a configured picture of the real car (`fixtures/car.png`, a painted stand-in) replacing the drawn car while charger, cable, road and bolt badge stay, and a picture that fails to load falling back to the drawing; a picture from the media library shown through the signed address of `media_source/resolve_media`, asked for once and not on every render, a vanished item falling back without a second request |
...
| `vd=0` | `vehicle_device=False` | Leave out the device of the vehicle's own integration (`fixtures/vehicle_device.json`, merged in by default with state ages in minutes instead of timestamps), so the vehicle mode runs on ha-evcc data alone. The devices ha-evcc creates per vehicle stay |
